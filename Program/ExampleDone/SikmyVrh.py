...
while running:
    ...
    if stav_hry in (NEODSTARTOVANA, TAHAM_ZA_KOLECKO, NASTAVUJI_SMER):
        ...
        textsurface = comic_sans_font.render("Nastav pocatecni pozici (leva mys) {:.2f} {:.2f} a smer (prava mys) {:.2f} {:.2f}: ".format(posx, posy, vx, vy), False, black)
        ...
        textsurface = comic_sans_font.render("Dopadl na zem a Dosahl max vysky {:.2f} a doletel {:.2f} daleko (restart r)".format(maxy, maxx), False, black)
        screen.blit(textsurface, dest=(60, 300))
